<template>
  <div class="main">
    ...
    <div class="itemResult" v-show="showCompany">
      <md-scroll-view
        auto-reflow
        :scrolling-x="false"
        :scrolling-y="true"
      >
        ...
        <div class="lable" v-for="(item,index) in companyList" :key="index"
             @click="$router.push({path:'/Company',query:{companyId:item.id}})"
        >
...
<!--                <div class="tag">国五</div>-->
    ...
    <div class="content">
      ...
      >
        ...
        <!--车辆列表-->
        <VcCard class="carItem"  v-for="(car,index) in carList" :key="`${car.id}_${index}`" :carData="car"></VcCard>
        <div v-show="carList.length===0" style="text-align: center;color:#ccc">
          当前没有该款车型！请尝试搜索其他车型
        </div>
    ...
    <Navigation v-bind:adc="qwe" :num="num"></Navigation>
...
<!--    品牌选择-->
    <md-landscape
      ...
      full-screen
    >
<!--      <mbrandPicker :show-box="true" @input="handleInput" @change="handleChange"></mbrandPicker>-->
...
<script>
    ...
    export default {
      ...
      methods:{
        async getBatchCompany(){
          const _data = await this.$api.post('/car/getBatchCompany',{
            keywords: this.searchTxt
          })
          const { data, success } = _data.data
          if(!success) return this.showCompany = false
        ...
        handleInput(res){
          ...
          this.nowOption.brandId = res[0]
          this.$refs.scrollView4.triggerRefresh()
          this.page = 1
          this.getList();
        ...
        doSearch(){
          this.page =1;
          this.carList = [];
          // 如果是有标题内容  那么brandName要重置
          if(this.searchTxt && this.brandName) this.brandName = ''
          this.getList()
        },
        doOk(){ //确认
          this.page =1;
        ...
        async getList(){
          ...
          //如果有 标题不要brandId
          if(this.searchTxt)  postData.brandId = ''
          postData.formState = 1
          if(this.page == 1) this.$toast.loading('1775正在努力加载中')
          try{
